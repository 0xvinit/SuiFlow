...
async function main() {
  ...
  const [arbSepolia] = await Promise.all([
    deployToChain(
      'Arbitrum Sepolia',
      process.env.SRC_CHAIN_RPC!,
      421614,
  ...
  console.log('🎉 DONE!')
  console.table({ arbSepolia })

  // Save results
  require('fs').writeFileSync('./deployments.json', JSON.stringify({ arbSepolia }, null, 2))
}
